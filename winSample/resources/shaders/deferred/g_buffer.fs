...
//layout (location = 4) out vec4 gEmissionMap;
in vec2 UV0;
in vec3 FragPos;
in mat3 TBN;
...
{    
    ...
    gPositionMetallic.a = texture(TexMetallic, UV0).r * Metallic;
    // also store the per-fragment (bump-)normals into the gbuffer
    float roughness = texture(TexRoughness, UV0).r * Roughness;
	...
	gAlbedoAO.rgb= pow(gAlbedoAO.rgb, vec3(2.2));  
    gAlbedoAO.a = texture(TexAO, UV0).r;
	//gEmissionMap.rgb=texture(TexEmission,UV0).rgb;
	//gEmissionMap.rgb= pow(gEmissionMap.rgb, vec3(2.2));  
