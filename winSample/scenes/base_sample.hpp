...
class GameEngineBaseApp
{	
protected:
	GameEngineBaseApp() {}
	GameEngineBaseApp(const GameEngineBaseApp& rhs) = delete;
	GameEngineBaseApp & operator=(const GameEngineBaseApp& rhs) = delete;
	virtual ~GameEngineBaseApp();

	static GameEngineBaseApp* mApp;
public:
	static GameEngineBaseApp* GetApp();

	virtual bool Initialize();

	virtual bool InitRender();

	virtual bool SetUpCallBack(GLFWframebuffersizefun frameBufferFunc, GLFWkeyfun keyFunc, GLFWcursorposfun mousePosFunc, GLFWmousebuttonfun mouseButtonFunc, GLFWscrollfun mouseScrollFunc);
...
public:
	...
	int SCR_WIDTH = 1920;
	int SCR_HEIGHT = 1080;
	std::string _WinTitle = "Hello CEngine";
	Cell::Renderer* renderer;
	Cell::FlyCamera camera(math::vec3f(0.0f, 1.0f, 0.0f), math::vec3f(0.0f, 0.0f, -1.0f));
};
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	// scene setup
	Cell::SceneNode* mainTorus = Cell::Scene::MakeSceneNode("mainTorus",&torus, matPbr);
	Cell::SceneNode* secondTorus = Cell::Scene::MakeSceneNode("secondTorus",&torus, matPbr);
	Cell::SceneNode* thirdTorus = Cell::Scene::MakeSceneNode("thirdTorus",&torus, matPbr);
	Cell::SceneNode* plasmaOrb = Cell::Scene::MakeSceneNode("plasmaOrb",&tSphere, matPlasmaOrb);
	...
	//secondTorus->SetLocalRotation(math::vec4f(0.0, 1.0, 0.0, math::Deg2Rad(90.0)));
	...
	sponza->SetLocalPosition(math::vec3f(0.0, -1.0, 0.0));
	sponza->SetLocalScale(0.01f);

	// lighting
	Cell::DirectionalLight dirLight;
	dirLight.Direction = math::vec3f(0.2f, -1.0f, 0.25f);
	dirLight.Color = math::vec3f(1.0f, 0.89f, 0.7f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/*	mainTorus->SetLocalRotation(math::vec4f(math::vec3f(1.0f, 0.0f, 0.0f), glfwGetTime() * 2.0));
			...
			thirdTorus->SetRotation(math::vec4f(math::vec3f(0.0f, 1.0f, 0.0f), glfwGetTime() * 4.0));
*/
...

		{
			...
			renderer->PushRender(plasmaOrb, time, frameIdx);
