...
{
    // --------------------------------------------------------------------------------------------
    FlyCamera::FlyCamera(math::vec3f position, math::vec3f forward, math::vec3f up) : Camera(position, forward, up)
    {
        Yaw = -90.0f;
    ...
    void FlyCamera::Update(float dt)
    {
        ...
        Position = math::vec3f::lerp(Position, m_TargetPosition, math::MathHelper::clamp01(dt * Damping));
        Yaw      = math::lerp(Yaw, m_TargetYaw, math::MathHelper::clamp01(dt * Damping * 2.0f));
        Pitch    = math::lerp(Pitch, m_TargetPitch, math::MathHelper::clamp01(dt * Damping * 2.0f));

        // calculate new cartesian basis vectors from yaw/pitch pair:
        math::vec3f newForward;
        ...
        Forward = math::vec3f::normalize(newForward);
        Right = math::vec3f::normalize(math::vec3f::crossProduct(Forward, m_WorldUp));
        Up = math::vec3f::crossProduct(Right, Forward);
