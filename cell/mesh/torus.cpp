...
namespace Cell
{
    ...
    Torus::Torus(float r1, float r2, unsigned int numSteps1, unsigned int numSteps2)
    {
        ...
        // loops.
        std::vector<math::vec3f> p(numSteps1 + 1);
        float a = 0.0f;
        ...
        for (int i = 0; i <= numSteps1; ++i)
        {
            ...
            // positive z-axis.
            math::vec3f u = math::vec3f::normalize(math::vec3f(0.0f) - p[i]) * r2; // Could be p[i] also        
            math::vec3f v = math::vec3f(0.0f, 0.0f, 1.0f) * r2;
            ...
            for (int j = 0; j <= numSteps2; ++j)
            {
                ...
                Normals[i * (numSteps2 + 1) + j] = math::vec3f::normalize(u * c + v * s);
                a += step;
            }
        }
