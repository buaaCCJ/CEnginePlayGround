...
namespace Cell
{
    ...
    PBR::PBR(Renderer* renderer)
    {
        ...
        m_SceneEnvCube = new SceneNode("m_SceneEnvCube");
		auto mr = m_SceneEnvCube->AddComponent<MeshRender>();
		mr->SetMaterial(m_PBRHdrToCubemap);
		mr->SetMesh(m_PBRCaptureCube);
    ...
    // --------------------------------------------------------------------------------------------
    void PBR::AddIrradianceProbe(PBRCapture* capture, math::vec3f position, float radius)
    {
    ...
    PBRCapture* PBR::ProcessEquirectangular(Texture* envMap)
    {
        ...
        m_SceneEnvCube->GetComponent<MeshRender>()->SetMaterial(m_PBRHdrToCubemap);
    ...
    PBRCapture* PBR::ProcessCube(TextureCube* capture, bool prefilter)
    {
		...
		captureProbe->originSkyBox = capture;
        // irradiance
		...
		m_SceneEnvCube->GetComponent<MeshRender>()->SetMaterial(m_PBRIrradianceCapture);
        m_Renderer->renderToCubemap(m_SceneEnvCube, captureProbe->Irradiance, math::vec3f(0.0f), 0);
        // prefilter 
        if (prefilter)
        {
			...
			m_SceneEnvCube->GetComponent<MeshRender>()->SetMaterial(m_PBRPrefilterCapture);
            ...
            for (unsigned int i = 0; i < maxMipLevels; ++i)
            {
                ...
                m_Renderer->renderToCubemap(m_SceneEnvCube, captureProbe->Prefiltered, math::vec3f(0.0f), i);
    ...
    // --------------------------------------------------------------------------------------------
    std::vector<PBRCapture*> PBR::GetIrradianceProbes(math::vec3f queryPos, float queryRadius)
    {
        // retrieve all irradiance probes in proximity to queryPos and queryRadius
        std::vector<PBRCapture*> capturesProximity;
        for (int i = 0; i < m_CaptureProbes.size(); ++i)
        {
            float lengthSq = ((m_CaptureProbes[i]->Position - queryPos)).lengthSq();
    ...
    void PBR::RenderProbes()
    {
        ...
        m_ProbeDebugShader->SetVector("Position", math::vec3f(0.0f, 2.0, 0.0f));
