...
namespace Cell
{
    ...
    // --------------------------------------------------------------------------------------------
    void CommandBuffer::Push(Mesh* mesh, Material* material, math::mat4f transform, math::mat4f prevTransform, math::vec3f boxMin, math::vec3f boxMax, RenderTarget* target)
    {
	...
	void CommandBuffer::Push(SceneNode * node,RenderTarget * target)
	{
		...
		if (!mr)
		{
			std::cerr << "can not find smr or mr" << std::endl;
			return;
		}
		command.Mesh = mr->GetMesh();
		command.Material = mr->GetMaterial();

		command.Transform = node->GetGlobalTransform();
	
		command.PrevTransform = node->GetGlobalPrevTransform();
		command.BoxMin = mr->GetBoundingBoxMin();
		command.BoxMax = mr->GetBoundingBoxMax();

		// if material requires alpha support, add it to alpha render commands for later rendering.
		auto material = mr->GetMaterial();
		if (material == nullptr)
		{
			std::cout << "material is nullptr,please check it" << std::endl;
			return;
		}
		if (material->Blend)
		{
    ...
    {       
        ...
        */ 
		//Note(ChenC) new added
		//auto aMat = a.Node->GetComponent<MeshRender>()->GetMaterial();
		//auto bMat = b.Node->GetComponent<MeshRender>()->GetMaterial();
		auto & aMat = a.Material;
		auto & bMat = b.Material;

        return std::make_tuple(aMat->Blend, aMat->GetShader()->ID) <
               std::make_tuple(bMat->Blend, bMat->GetShader()->ID);
    }
