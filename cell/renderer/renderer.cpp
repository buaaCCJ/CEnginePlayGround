...
namespace Cell
{
	...
	void Renderer::Init(GLADloadproc loadProcFunc)
	{
		...
		Cell::Texture *hdrMap = Cell::Resources::LoadHDR("sky env", "textures/backgrounds/newport_loft.hdr");
	...
	// ------------------------------------------------------------------------
	math::vec2f Renderer::GetRenderSize()
	{
		return m_RenderSize;
	}
	// ------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------
	void Renderer::PushRender(Mesh* mesh, Material* material, math::mat4f transform, math::mat4f prevFrameTransform)
	{
		...
		m_CommandBuffer->Push(mesh, material, transform, prevFrameTransform, math::vec3f(-99999.0f), math::vec3f(99999.0f), target);
	}
	// ------------------------------------------------------------------------
	void Renderer::PushRender(SceneNode* node, double time, uint64_t frameIdx)
	{

		//need refactor
		node->Update(time, frameIdx);
		...
		while (!nodeStack.empty())
		{
			...
			if (!mr)
				mr = node->GetComponent<SkinningMeshRender>();
			if (mr)
			{
				math::vec3f boxMinWorld = node->GetWorldPosition() + (node->GetWorldScale() * mr->GetBoundingBoxMin());
				math::vec3f boxMaxWorld = node->GetWorldPosition() + (node->GetWorldScale() * mr->GetBoundingBoxMax());
				mr->SetBoundingBox(boxMinWorld, boxMaxWorld);
				m_CommandBuffer->Push(node, target);
			}
			for (unsigned int i = 0; i < node->GetChildCount(); ++i)
				nodeStack.push(node->GetChildByIndex(i));
		}
	...
	void Renderer::RenderPushedCommands()
	{
	   ...
	   // std::vector<RenderCommand> deferredRenderCommands = m_CommandBuffer->GetDeferredRenderCommands(true);
		std::vector<RenderCommand> deferredRenderCommands = m_CommandBuffer->GetDeferredRenderCommands(false);
		...
		if (Shadows)
		{
			...
			for (int i = 0; i < m_DirectionalLights.size(); ++i)
			{
				...
				if (light->CastShadows)
				{
					...
					math::mat4f lightProjection = math::mat4f::createOrtho(-20.0f, 20.0f, 20.0f, -20.0f, -15.0f, 20.0f);
					math::mat4f lightView = math::mat4f::createLookAt(-light->Direction * 10.0f, math::vec3f(0.0), math::vec3f(0, 1, 0));
		...
		//m_GBuffer->GetColorTexture(3)->Bind(3);
		...
		for (unsigned int targetIndex = 0; targetIndex < m_RenderTargetsCustom.size(); ++targetIndex)
		{
			...
			std::vector<RenderCommand> renderCommands = m_CommandBuffer->GetCustomRenderCommands(renderTarget,false);
		...
		for (auto it = m_PointLights.begin(); it != m_PointLights.end(); ++it)
		{
			if ((*it)->RenderMesh)
			{
				...
				math::mat4f model;
				math::Quatf q;
				model = math::mat4f::Compose(math::vec3f(0.25f), q, (*it)->Position);
				/*  math::translate(model, (*it)->Position);
				  math::scale(model, math::vec3f(0.25f));*/
		...
		if (LightVolumes)
		{
			...
			for (auto it = m_PointLights.begin(); it != m_PointLights.end(); ++it)
			{
				...
				math::mat4f model;
				math::Quatf q;
				model = math::mat4f::Compose(math::vec3f((*it)->Radius), q, (*it)->Position);
				/*math::translate(model, (*it)->Position);
				math::scale(model, math::vec3f((*it)->Radius));*/
				command.Transform = model;
	...
	void Renderer::ReadPixels(const std::string & path)
	{
	...
	// ------------------------------------------------------------------------
	void Renderer::AddIrradianceProbe(math::vec3f position, float radius)
	{
		m_ProbeSpatials.push_back(math::vec4f(position, radius));
	}
	// ------------------------------------------------------------------------
	void Renderer::BakeProbes(SceneNode* scene)
	{
		...
		while (!sceneStack.empty())
		{
			...
			sceneStack.pop();
			if (node->GetComponent<MeshRender>())
			{
				auto mat = node->GetComponent<MeshRender>()->GetMaterial();
				auto samplerUniforms = *(mat->GetSamplerUniforms());
				if (samplerUniforms.find("TexAlbedo") != samplerUniforms.end() && samplerUniforms.find("TexAlbedo")->second.isDirty)
				{
					...
					if (samplerUniforms.find("TexNormal") != samplerUniforms.end() && samplerUniforms.find("TexNormal")->second.isDirty)
					{
						materials[materials.size() - 1]->SetTexture("TexNormal", samplerUniforms["TexNormal"].Texture, 1);
					}
					if (samplerUniforms.find("TexMetallic") != samplerUniforms.end() && samplerUniforms.find("TexMetallic")->second.isDirty)
					{
						materials[materials.size() - 1]->SetTexture("TexMetallic", samplerUniforms["TexMetallic"].Texture, 2);
					}
					if (samplerUniforms.find("TexRoughness") != samplerUniforms.end() && samplerUniforms.find("TexRoughness")->second.isDirty)
					{
					...
					//node->GetComponent<MeshRender>()->SetMaterial(materials[materials.size() - 1]);
					commandBuffer.Push(mesh, materials[materials.size() - 1], node->GetGlobalTransform());
				}
				...
				{   // we have a background scene node, add those as well
					...
					materials[materials.size() - 1]->DepthCompare = mat->DepthCompare;
					//node->GetComponent<MeshRender>()->SetMaterial(materials[materials.size() - 1]);
					auto mesh = node->GetComponent<MeshRender>()->GetMesh();
					commandBuffer.Push(mesh, materials[materials.size() - 1], node->GetGlobalTransform());
				}
		...
		for (int i = 0; i < m_ProbeSpatials.size(); ++i)
		{
			...
			math::vec3f vec(m_ProbeSpatials[i].x, m_ProbeSpatials[i].y, m_ProbeSpatials[i].z);
			renderToCubemap(renderCommands, &renderResult, vec);

			PBRCapture* capture = m_PBR->ProcessCube(&renderResult, false);
			m_PBR->AddIrradianceProbe(capture, vec, m_ProbeSpatials[i].w);
		}
	...
	void Renderer::renderCustomCommand(RenderCommand* command, Camera* customCamera, bool updateGLSettings)
	{
		...
		if (command->Node && command->Node->GetName() == "_body")
		{
			//std::cout << "ccj: test" << std::endl;
		}
		...
		material->GetShader()->SetBool("isSkinning", false);

		if (mesh->bIsHaveBone)
		{
			auto isSuit = mesh->isSuitGPUSkin();
			//isSuit = false;
			if (isSuit)
			{
				SkinningMeshRender* smr = command->Node->GetComponent<SkinningMeshRender>();
				if (!smr)
				{
					std::cerr << "can not find smr" << std::endl;
					return;
				}
				const std::vector<float>& floatArray = smr->GetSkinningMat();
				if (floatArray.size() > 0)
				{
					material->GetShader()->SetBool("isSkinning", true);

					material->GetShader()->SetFloatArray("skinningMatrix", floatArray);
				}
			}
			else
			{
		...
		{
			it->second->UpLoad(material->GetShader());
		}
	...
	// ------------------------------------------------------------------------
	void Renderer::renderToCubemap(std::vector<RenderCommand>& renderCommands, TextureCube* target, math::vec3f position, unsigned int mipLevel)
	{
		// define 6 camera directions/lookup vectors
		Camera faceCameras[6] = {
			Camera(position, math::vec3f(1.0f,  0.0f,  0.0f), math::vec3f(0.0f, -1.0f,  0.0f)),
			Camera(position, math::vec3f(-1.0f,  0.0f,  0.0f), math::vec3f(0.0f, -1.0f,  0.0f)),
			Camera(position, math::vec3f(0.0f,  1.0f,  0.0f), math::vec3f(0.0f,  0.0f,  1.0f)),
			Camera(position, math::vec3f(0.0f, -1.0f,  0.0f), math::vec3f(0.0f,  0.0f,-1.0f)),
			Camera(position, math::vec3f(0.0f,  0.0f,  1.0f), math::vec3f(0.0f, -1.0f,  0.0f)),
			Camera(position, math::vec3f(0.0f,  0.0f, -1.0f), math::vec3f(0.0f, -1.0f,  0.0f))
		};
		/*
			std::vector<Camera>faceCameras;
			faceCameras.push_back(Camera());*/
	...
	{
		glBindBuffer(GL_UNIFORM_BUFFER, m_GlobalUBO);
		glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(math::mat4f), m_Camera->Projection * m_Camera->View); // sizeof(math::mat4f) = 64 bytes
		glBufferSubData(GL_UNIFORM_BUFFER, 64, sizeof(math::mat4f), m_PrevViewProjection);
		glBufferSubData(GL_UNIFORM_BUFFER, 128, sizeof(math::mat4f), m_Camera->Projection);
		glBufferSubData(GL_UNIFORM_BUFFER, 192, sizeof(math::mat4f), m_Camera->View);
		glBufferSubData(GL_UNIFORM_BUFFER, 256, sizeof(math::mat4f), m_Camera->View); // TODO: make inv function in math library
		// scene data
		glBufferSubData(GL_UNIFORM_BUFFER, 320, sizeof(math::vec4f), &m_Camera->Position[0]);
		// lighting
		unsigned int stride = 2 * sizeof(math::vec4f);
		for (unsigned int i = 0; i < m_DirectionalLights.size() && i < 4; ++i) // no more than 4 directional lights
		{
			glBufferSubData(GL_UNIFORM_BUFFER, 336 + i * stride, sizeof(math::vec4f), &m_DirectionalLights[i]->Direction[0]);
			glBufferSubData(GL_UNIFORM_BUFFER, 336 + i * stride + sizeof(math::vec4f), sizeof(math::vec4f), &m_DirectionalLights[i]->Color[0]);
		}
		for (unsigned int i = 0; i < m_PointLights.size() && i < 8; ++i) //  constrained to max 8 point lights in forward context
		{
			glBufferSubData(GL_UNIFORM_BUFFER, 464 + i * stride, sizeof(math::vec4f), &m_PointLights[i]->Position[0]);
			glBufferSubData(GL_UNIFORM_BUFFER, 464 + i * stride + sizeof(math::vec4f), sizeof(math::vec4f), &m_PointLights[i]->Color[0]);
		}
	}
	// --------------------------------------------------------------------------------------------
		...
		if (IrradianceGI && irradianceProbes.size() > 0)
		{
			...
			for (int i = 0; i < irradianceProbes.size(); ++i)
			{
				...
				if (m_Camera->Frustum.Intersect(probe->Position, probe->Radius))
				{
					...
					math::mat4f model;
					math::Quatf q;
					model = math::mat4f::Compose(math::vec3f(probe->Radius), q, probe->Position);
					/* math::translate(model, probe->Position);
					 math::scale(model, math::vec3f(probe->Radius));*/
	...
	void Renderer::renderDeferredDirLight(DirectionalLight* light)
	{
		...
		dirShader->SetVector("lightColor", math::vec3f::normalize(light->Color) * light->Intensity);
	...
	void Renderer::renderDeferredPointLight(PointLight* light)
	{
		...
		pointShader->SetVector("lightColor", math::vec3f::normalize(light->Color) * light->Intensity);

		math::mat4f model;
		math::Quatf q;
		model = math::mat4f::Compose(math::vec3f(light->Radius), q, light->Position);
		/* math::translate(model, light->Position);
		 math::scale(model, math::vec3f(light->Radius));*/
		pointShader->SetMatrix("model", model);

		renderMesh(m_DeferredPointMesh, pointShader);
	}
	// --------------------------------------------------------------------------------------------
	void Renderer::renderShadowCastCommand(RenderCommand* command, const math::mat4f& projection, const math::mat4f& view)
	{
		//Note(ChenC)
		//write  mesh back face depth(in light space ) into frame buffer
