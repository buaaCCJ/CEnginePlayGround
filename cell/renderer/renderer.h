...
namespace Cell
{
    ...
    */
    ...
    private:       
        ...
        // camera
        Camera*    m_Camera;
        math::mat4f m_PrevViewProjection;
        ...
        std::vector<math::mat4f>    m_ShadowViewProjections;
       
        // pbr
        PBR* m_PBR;
        unsigned int m_PBREnvironmentIndex;
        std::vector<math::vec4f> m_ProbeSpatials;
    ...
    public:
        ...
        void PushRender(Mesh* mesh, Material* material, math::mat4f transform = math::mat4f(), math::mat4f prevFrameTransform = math::mat4f());
        void PushRender(SceneNode* node,double time,uint64_t frameIdx);
		...
		void ReadPixels(const std::string& path);

        // pbr
        void        SetSkyCapture(PBRCapture* pbrEnvironment);
        PBRCapture* GetSkypCature();
        void        AddIrradianceProbe(math::vec3f position, float radius);
    ...
    private:
        ...
        void renderToCubemap(SceneNode* scene, TextureCube* target, math::vec3f position = math::vec3f(0.0f), unsigned int mipLevel = 0);
        void renderToCubemap(std::vector<RenderCommand>& renderCommands, TextureCube* target, math::vec3f position = math::vec3f(0.0f), unsigned int mipLevel = 0);
        ...
        void renderShadowCastCommand(RenderCommand* command, const math::mat4f& projection, const math::mat4f& view);
    };
}
